...
public sealed class TutorialRuntime : MonoBehaviour
{
    ...
    private static bool ValidationMode => Array.IndexOf(Environment.GetCommandLineArgs(), "-ptmeoh-validate") >= 0;

    [RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.AfterSceneLoad)]
    private static void AutoCreate()
    {
        if (FindAnyObjectByType<TutorialRuntime>() != null) return;
    ...
    private void Start()
    {
        ...
        BuildSteps();
        BuildOverlay();
        if (ValidationMode || PlayerPrefs.GetInt(CompletedKey, 0) == 0) StartCoroutine(AutoStartWhenReady());
    }
    ...
    private void BuildSteps()
    {
        ...
        Add("THE MAIN NAVIGATION",
            "Six sections live in the header: OVERVIEW, PLANT PROCESS, FLOW LAB, REACTOR LAB, ANALYTICS and SIMULATION. Selecting one opens its panel and moves the camera to a matching view, and the highlighted button always shows where you are.\n\n" +
        ...
        Add("HIGH-OUTPUT PRESET",
            "SET MAXIMUM EFFICIENCY applies a predefined high-output operating point in this educational model, and resumes the run.\n\n" +
            "This is not a global optimisation result. Overall efficiency is a material-recovery proxy, not an energy-efficiency measurement. The storage safety interlock still applies.",
        ...
        Add("PLANT PROCESS",
            "PLANT PROCESS is a guided ten-step walkthrough, from renewable power and water all the way to methanol storage.\n\n" +
        ...
        Add("ANALYTICS - PROCESS STATE",
            "ANALYTICS opens the dedicated right-side pane, leaving the plant visible on the left. PAUSE and RESET sit in that title bar, so you can control the run without closing the window.\n\n" +
            "The PROCESS STATE tab shows the five headline metrics as bars: overall efficiency, CO2 capture, reactor yield, methanol purity and storage fill. Hover any bar to read its exact live value.",
            "Analytics Window",
            apply: () => Dashboard(d => d.TutorialSetAnalyticsView(true, null)));

        Add("ANALYTICS - EXPLORE",
            "The EXPLORE tab holds four graph views:\n\n" +
        ...
        Add("OFAT TIMELINE",
            "OFAT TIMELINE automatically evaluates 13 hypothetical points when you select Temperature, Pressure, H2/CO2, GHSV or Feed Flow. Only the selected factor varies; live process inputs stay unchanged. Select Free or another response to return to the recorded timeline.\n\n" +
            "The selected factor remains the only unlocked reactor slider while this tab is open. LINE ONLY and WITH POINTS change the plot style; timeline paging and EXPORT are also available.",
        ...
        Add("SAFETY AND EFFICIENCY WARNINGS",
            "The arrow points at the warning strip below the header, across the upper centre of the screen. That is where an amber caution or red critical band appears when an operating condition drifts out of the safe or sensible range - reactor temperature or pressure too high, storage nearly full, capture efficiency too low - listing the active alarms and cautions.\n\n" +
            "The strip is hidden when there are no active warnings. The band clears itself again as soon as conditions recover, so it is worth watching while you experiment with the sliders.",
            "Warning Panel",
            apply: () => Dashboard(d => d.TutorialShowPage("overview")),
